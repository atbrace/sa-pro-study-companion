...
interface AssessmentPageProps {
  params: { exam: string; domain: string };
...
export default function AssessmentPage({ params, searchParams }: AssessmentPageProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/questions?exam=${params.exam}&domain=${params.domain}${searchParams.topic ? `&topic=${searchParams.topic}` : ''}`)
      .then(res => res.json())
      .then(data => setQuestions(data.questions || []))
      .catch(err => console.error('Failed to load questions:', err));
  }, [params.exam, params.domain, searchParams.topic]);
    ...
    return (
      <div className="container py-8 max-w-4xl">
                  ...
                  <div key={area.topicId} className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Button variant="outline" size="sm" asChild>
                        <a href={`/${params.exam}/study/${area.domainId}/${area.topicId}`}>Review Topic</a>
                      </Button>
                    </div>
        ...
        <div className="flex gap-4">
          <Button onClick={() => router.push(`/${params.exam}/assess`)}>Back to Assessments</Button>
